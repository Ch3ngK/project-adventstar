...
export async function GET(request: Request) {
    ...
    const backendResponse = await fetch(apiUrl(`/leads`), {
        headers: {
...
export async function POST(request: Request) {
    ...
    if (!success) {
        return NextResponse.json(
            { message: "Too many leads requests. Please wait a moment." },
            { status: 429 }
        );
    }
    ...
    try {
        ...
        data = { message: "Lead generation failed. Please try again."}; 
    }
